...
- The API is designed to work with integer connection ID that is easy to use from C API or Java wrapper generated by GlueGen, or a component for a higher-level framework such as an Apache Cordova or React Native.
...
- 64-bit integer values are not supported by `object-batch-connection` as used by Cordova demo and intended to be used with React Native. The Cordova demo is tested with integer values up to 52-bits (signed).
